...
*Important Note* - 
1. EMAIL FUNCTIONALITY IS IMPLEMENTED - To receive emails in case of book request, create new account with valid email address. You can request notification for a checked out book. When book is returned by user, email will be sent to the user who has requested notification. Notification can be cancelled any time. Right now, only one user can request notification for a book at a time
